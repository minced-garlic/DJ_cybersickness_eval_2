...
dataset_x_dir= 'no_spectrogram_infoICA_rest_percent/'
...
def image_CNN(x_train, x_test, ps_train,  ps_test, sc_train,  sc_test, score_train,score_test,categorizes=None):
        x_train, x_test= data_nor_or_std(x_train,x_test,stand=stand)
        ...
        if num_classes == 1:
            ps_train, ps_test = regrassion_scaling(ps_train, ps_test)
            sc_train, sc_test = regrassion_scaling(sc_train, sc_test)
            ...
            resnet18.fit(x_train, score_train, steps_per_epoch=10, validation_steps=2, epochs=epochs, validation_data=(x_test, score_test),
                          callbacks=[mc2 ])
            ...
            sc_test = keras.utils.to_categorical(sc_test, categorizes[1]+1)
            score_train = keras.utils.to_categorical(score_train, 5)
            score_test = keras.utils.to_categorical(score_test, 5)
            ...
            filename2 = 'C:\\Users\\Admin\\PycharmProjects\\DJ_cybersickness_eval_2//models/model_ver' + \
                        time_start + '_2(epochs_' + str(epochs) + '_num_classes_' + str(num_classes) + ').h5'
            ...
            load_model1 = load_model(filename1)
            load_model2 = load_model(filename2)
            model1 = Model(inputs=load_model1.inputs, outputs=load_model1.layers[len(resnet18.layers) - 2].output)
            model2 = Model(inputs=load_model2.inputs, outputs=load_model2.layers[len(resnet18.layers) - 2].output)
    ...
    if not average_score:
        ...
        if num_classes>1:
            data_t_x, data_x, data_t_ps, data_ps, data_t_scene, data_scene,score_train ,score_test, categorizes,= \
                dataset_import.dataset_label_n_score2(dataset_dir+dataset_x_dir, testing = testing)
        else:
            data_t_x, data_x, data_t_ps, data_ps, data_t_scene, data_scene,data_score= \
                dataset_import.dataset_label_n_score(dataset_dir+dataset_x_dir,testing = testing)

        if (len(data_x) != len(data_ps)):
            print('data x(test x)의 길이 {}와 data y(test y)의 길이 {}가 같지 않습니다.'.format(len(data_x), len(data_ps)))
        else:
            image_CNN(data_t_x, data_x, data_t_ps, data_ps, data_t_scene, data_scene, score_train, score_test,
                      categorizes)
