...
def start(dataDir, filelist):
    ...
    eeg_lines_t = []
    if os.path.isfile(
            'D:\\2019임상데이터\\원본\\experimental_data\\' + filelist + '\\' + filelist + '\\SSQ\\' + filelist + '_Score.csv'):
        ...
        if not os.path.isfile('D:\\2019_data_for_Cnn\\data_cut_scene_npy\\' + filelist +'_'+score_learn[0][0]+'_'+score_learn[0][1]  +'.npy'):
            ...
            for lines in lines:
                ...
                if (len(cut_line)-1> 2*i and 52>i and len(score_learn)>i) and isNumber(lines[0]) and isNumber(lines[1]) :
                    if cut_line[2*i] <=float(lines[0]) and cut_line[2*i+1]>float(lines[0]):
                        ...
                        print('D:\\2019_data_for_Cnn\\data_cut_scene\\' + filelist +'_'+score_learn[i][0]+'_'+score_learn[i][1]  +'.npy')
                        np.save('D:\\2019_data_for_Cnn\\data_cut_scene_npy\\' + filelist +'_'+score_learn[i][0]+'_'+score_learn[i][1]  +'.npy',x)
        ...
        warnings.warn(filelist+' dont have score file')
